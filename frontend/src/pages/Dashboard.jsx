...
import { Activity, Thermometer, AlertTriangle, CheckCircle } from 'lucide-react';
      ...
      <header className="flex flex-col md:flex-row justify-between items-end mb-12">
        ...
        <div className="flex items-center gap-4 mt-4 md:mt-0">
           ...
           <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full border border-black/5 shadow-sm">
              <div className={`w-3 h-3 rounded-full ${status === 'nominal' ? 'bg-green-500 animate-pulse' : 'bg-red-500 animate-ping'}`}></div>
              <span className={`font-bold uppercase text-sm ${status === 'nominal' ? 'text-green-600' : 'text-red-600'}`}>
                {status}
              </span>
           </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <Card className={`md:col-span-1 flex flex-col justify-center transition-colors duration-500 ${status === 'anomaly' ? 'bg-red-600 text-white border-none' : ''}`}>
           <div className="flex items-center gap-4">
              {status === 'nominal' ? <CheckCircle size={32} /> : <AlertTriangle size={32} />}
              <div>
                 <div className="text-xs opacity-70 uppercase tracking-widest mb-1">AI Confidence</div>
                 <div className="text-2xl font-bold">{status === 'nominal' ? '99.8% Secure' : 'ANOMALY DETECTED'}</div>
              </div>
           </div>
        </Card>
        ...
        <Card>
          ...
          <div className="text-5xl font-light tracking-tighter">
            {reading.temp}<span className="text-lg text-gray-400 ml-1">°C</span>
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="text-5xl font-light tracking-tighter">
            {reading.vib}<span className="text-lg text-gray-400 ml-1">G</span>
          </div>
        </Card>
      </div>
